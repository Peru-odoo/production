...
class product_compination_account_move_surgi(models.Model):
    ...
    #compination_stock_pick = fields.Many2one("stock.picking")
...
class product_items_move_surgi(models.Model):
   ...
   # move_id = fields.Many2one("account.move", readonly='false')
    #Stock_pick_id=fields.Many2one("stock.picking", readonly='false')
    sale_order_id=fields.Many2one("sale.order",readonly='false')
    itemprice=participant_nick = fields.Float(string='Unit Price',)
...
class compined_sales_order(models.Model):
    ...
    def changed_line_ids(self):
        productslines = {}  # will hold products and there quantities
        ...
        for rec in self:
            ...
            if rec.order_line:
                ...
                tempcompinations = self.env['product.compination.groups.surgi'].search(
                    [('product_group', 'in', groups)])  # check which compination is exist in the products
                ...
                for v in compinations:
                    self._cr.execute(
                        "select product_group,product_quantity,product_compination_id from product_compination_groups_surgi where product_compination_id=%d" %
                        v['id'])
                ...
                for comp in compions:
                        ...
                        if k != False :
                            if products[k]['totalq'] < compions[comp]['groups'][k]['quantity'] or compions[comp]['groups'][k]['quantity']<=0:
                                ...
                                if itemsno == -1 or itemsno > cx:
                                    itemsno = cx
                                    compinationprice+=itemsno *products[k]['price']
                    ...
                    if accepted:
                        ...
                        compions[comp]['price'] = compinationprice
                ...
                for p in products:
                    if products[p]['totalq'] > 0:
                        print(p)
                        vbvb = products[p]['totalq']
                        nbnbnb = str(products[p]['pro_name'])
                        # self._cr.execute(
                        #   'insert into product_item_movement_surgi  (product_count,Stock_pick_id,product_name) values (%d,%d,%s)  ' % (
                        #      products[p]['totalq'], moveid, "'" + str(products[p]['pro_name']) + "'"))
                        self.env['product.item.movement.surgi'].create({
                            'product_count': products[p]['totalq'],
                            'sale_order_id': moveid,
                            'product_name': str(products[p]['pro_name']),
...
class sales_order_line_inhert1(models.Model):
    _inherit="sale.order.line"
    pro_group = fields.Char(string="Group", related="product_id.product_tmpl_id.product_group")
...
class sale_order_line_inhert(models.Model):
    ...
    # groupx=fields.Char(string="My new group",related='pro_tem.product_group')
    pass
class stock_move_compined_inhert(models.Model):
    #_name = "sale.order.line"
    _inherit = "stock.move"
    pro_group = fields.Char(string="Group", related="product_id.product_tmpl_id.product_group")
    ...
    # groupx=fields.Char(string="My new group",related='pro_tem.product_group')
    pass
# class stock_picking_inhert_3(models.Model):
#     _inherit="stock.picking"
#     cominations_id = fields.One2many("product.compination.movement.surgi", string="Compinations",inverse_name="compination_stock_pick")
#     items_move_id = fields.One2many('product.item.movement.surgi', string="Products", inverse_name="Stock_pick_id")
#
#     #@api.model
#     def changed_line_ids(self):
#         products = {}  # will hold products and there quantities
...
#         self.changed_line_ids()
#         print("x")
#         res = super(stock_picking_inhert_3, self).button_validate()
#
#         return res
#         pass
#     pass
