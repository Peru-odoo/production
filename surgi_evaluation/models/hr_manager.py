...
class HRManagerEvaluation(models.Model):
    ...
    def check_if_department_manager(self):
        ...
        for rec in self:
            if self.env.user.id == rec.employee_id.parent_id.parent_id.user_id.id or self.env.user.id == rec.employee_id.parent_id.parent_id.parent_id.user_id.id or self.env.user.id == rec.employee_id.parent_id.parent_id.parent_id.parent_id.user_id.id:
    ...
    def button_parent_confirm(self):
        self.state='done'