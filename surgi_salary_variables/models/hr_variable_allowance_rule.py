...
class HrVariableAllowanceRule(models.Model):
    ...
    administrator_ids = fields.Many2many(comodel_name="res.users", string="Administrator", )

    need_approve = fields.Boolean(string="Need Approve")
...
"""
        if not self.salary_rule_id:
            salary_rule_id = self.env['hr.salary.rule'].create({
                'name': self.name,
                'category_id': self.env.ref('surgi_salary_variables.ALW').id if self.allowance_or_deduction == 'allowance' else self.env.ref('surgi_hr_variable_allowance.DED').id,
                'code': self.code,
                'sequence': 5,
                'struct_id': self.env.ref('surgi_salary_variables.variable_allowance_salary_structure').id,
                'active': True,
